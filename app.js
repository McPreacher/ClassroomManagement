...
function printVerseReport() {
    const today = new Date().toLocaleDateString();
    const classData = students[currentClass];
    
    // Filter for only students who have behavior marks (verses) today
    const studentsWithVerses = classData.filter(s => s.behaviorMarks > 0);

    if (studentsWithVerses.length === 0) {
        alert("No students have verses due today!");
        return;
    }
    ...
    let reportHtml = `
        <html>
        <head>
            <title>Verse Report - ${today}</title>
            <style>
        ...
        <body>
            <h1>Verse Report: ${currentClass}</h1>
            <p>Date: ${today}</p>
            <table>
                <thead>
                    <tr>
                        ...
                        <th>Total Verses Due Today</th>
                    </tr>
                </thead>
                <tbody>
                    ${studentsWithVerses.map(s => `
                        <tr>
                            <td>${s.name}</td>
                            <td>${s.behaviorMarks}</td>
                            <td><strong>${s.behaviorMarks * 10} Verses</strong></td>
                        </tr>
                    `).join('')}
                </tbody>
            </table>
            <div class="footer">Generated by DCS Classroom Management</div>
        </body>
        </html>
    `;
